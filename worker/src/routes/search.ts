...
searchRoutes.get('/', async (c) => {
  ...
  const [tasks, shopping, inventory, finances, memories] = await Promise.all([
    supabase.from('tasks').select('id,title,description,status,created_at').eq('user_id', user.id).or(`title.ilike.%${query}%,description.ilike.%${query}%`).limit(10),
    supabase.from('shopping_items').select('id,name,category,purchased,created_at').eq('user_id', user.id).or(`name.ilike.%${query}%,category.ilike.%${query}%`).limit(10),
    supabase.from('inventory').select('id,name,category,location,created_at').eq('user_id', user.id).or(`name.ilike.%${query}%,category.ilike.%${query}%`).limit(10),
    supabase.from('finances').select('id,description,category,type,amount,created_at').eq('user_id', user.id).or(`description.ilike.%${query}%,category.ilike.%${query}%`).limit(10),
    supabase.from('memories').select('id,key,value,category,created_at').eq('user_id', user.id).or(`key.ilike.%${query}%,value.ilike.%${query}%`).limit(10),
  ])
